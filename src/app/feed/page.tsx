...
const TIPSTERS = [
  { id: "vh",  name: "ValueHunter",  initials: "VH", color: "#00e5a0", verified: true,  yield: 12.4, subs: 1840, tips: 612  },
  { id: "xg",  name: "xG_Machine",   initials: "XG", color: "#5ad1ff", verified: true,  yield: 9.8,  subs: 1320, tips: 488  },
  { id: "ace", name: "AceServe",      initials: "AS", color: "#e6b23a", verified: true,  yield: 8.1,  subs: 760,  tips: 295  },
  { id: "gr",  name: "GreenRoom",     initials: "GR", color: "#b48cff", verified: false, yield: 6.5,  subs: 410,  tips: 188  },
  { id: "tt",  name: "TravTanten",    initials: "TT", color: "#ff7a7a", verified: true,  yield: 5.2,  subs: 980,  tips: 522  },
  { id: "sl",  name: "SharpLines",    initials: "SL", color: "#00e5a0", verified: true,  yield: 4.9,  subs: 540,  tips: 210  },
];
...
const FEED = [
  {
    tipster: TIPSTERS[0], sport: "🎾", pro: false,
    ...
    bookmaker: "Unibet", postedYield: 12.4, posted: "2 hr ago", locked: false,
  },
  {
    tipster: TIPSTERS[1], sport: "🎾", pro: true,
    league: "Roland Garros · Men's Final", match: "Jannik Sinner – Carlos Alcaraz", date: "Sun 8 Jun · 15:00 CET",
    ...
    bookmaker: "Pinnacle", postedYield: 9.8, posted: "1 hr ago", locked: true,
  },
  {
    tipster: TIPSTERS[2], sport: "🏀", pro: false,
    ...
    bookmaker: "Bet365", postedYield: 8.1, posted: "3 hr ago", locked: false,
  },
  {
    tipster: TIPSTERS[3], sport: "⚽", pro: false,
    ...
    bookmaker: "Betsson", postedYield: 6.5, posted: "5 hr ago", locked: false,
  },
  {
    tipster: TIPSTERS[5], sport: "⚽", pro: true,
    ...
    bookmaker: "William Hill", postedYield: 4.9, posted: "6 hr ago", locked: true,
  },
  {
    tipster: TIPSTERS[4], sport: "🐎", pro: false,
    ...
    bookmaker: "ATG", postedYield: 5.2, posted: "1 hr ago", locked: false,
  },
  {
    tipster: TIPSTERS[0], sport: "⚽", pro: false,
    ...
    bookmaker: "Unibet", postedYield: 12.4, posted: "20 min ago", locked: false,
  },
  {
    tipster: TIPSTERS[2], sport: "🎾", pro: false,
    ...
    bookmaker: "NordicBet", postedYield: 8.1, posted: "45 min ago", locked: false,
  },
];

const SPORTS_FILTER = ["All", "⚽ Football", "🎾 Tennis", "🐎 Trotting", "⛳ Golf"];
const TIER_FILTER   = ["All", "Free", "Pro"];

// ── Component ──────────────────────────────────────────────

export default function FeedPage() {
  const { format, stakeLabel } = useCurrency();
  const [sportFilter, setSportFilter] = useState("All");
  const [tierFilter, setTierFilter]   = useState("All");

  const filtered = FEED.filter(tip => {
    const sportOk = sportFilter === "All" || tip.sport === sportFilter.split(" ")[0];
    const tierOk  = tierFilter  === "All" || (tierFilter === "Pro" ? tip.pro : !tip.pro);
    return sportOk && tierOk;
  });
    ...
    <div className="fade-in">
      ...
      <div className="panel" style={{ padding: "12px 16px", marginBottom: 20, display: "flex", alignItems: "center", gap: 16, flexWrap: "wrap" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          ...
          <Segmented options={TIER_FILTER} value={tierFilter} onChange={setTierFilter} accent />
        </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "2fr 1fr", gap: 16, alignItems: "start" }}>
        ...
        <div className="panel">
          ...
          {filtered.length === 0 ? (
            ...
            filtered.map((tip, i) => (
              <FeedRow key={i} tip={tip} format={format} stakeLabel={stakeLabel} />
            ))
          )}
        </div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
                ...
                >
                  ...
                  <div>
                    ...
                    <div style={{ color: "var(--text-3)", fontFamily: "var(--mono)", fontSize: 10.5, marginTop: 1 }}>
                      {t.subs.toLocaleString()} subs · {t.tips} tips
                    </div>
                  </div>
          ...
          <div className="panel" style={{ padding: "var(--pad)" }}>
            <div style={{ fontWeight: 600, fontSize: 14, marginBottom: 6 }}>Become a tipster</div>
            <div style={{ color: "var(--text-2)", fontSize: 13, lineHeight: 1.6, marginBottom: 16 }}>
              Publish your picks, build a following, and monetize your edge.
            </div>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginBottom: 16 }}>
              {[
                { label: "Your share", val: "80%" },
                { label: "Avg price", val: "€9 / mo" },
              ].map(s => (
                <div key={s.label} style={{ padding: "10px 12px", background: "var(--bg-2)", borderRadius: "var(--r-s)", border: "1px solid var(--line)" }}>
                  <div className="label" style={{ marginBottom: 4 }}>{s.label}</div>
                  <div className="num pos glow" style={{ fontSize: 18, fontWeight: 600 }}>{s.val}</div>
                </div>
              ))}
            </div>
...
function FeedRow({ tip, format, stakeLabel }: {
  tip: typeof FEED[0];
    ...
    >
      ...
      <div>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 4, flexWrap: "wrap" }}>
          ...
          {tip.pro
            ? <span className="tag accent" style={{ fontSize: 10, padding: "1px 6px" }}>PRO</span>
            : <span className="tag"        style={{ fontSize: 10, padding: "1px 6px" }}>FREE</span>
          }
          <span style={{ color: "var(--text-3)", fontFamily: "var(--mono)", fontSize: 11, marginLeft: 2 }}>
        ...
        <div style={{
          ...
          overflow: "hidden",
          position: "relative",
          ...(tip.locked ? { filter: "blur(5px)", userSelect: "none", pointerEvents: "none" } : {}),
        }}>
        ...
        {tip.locked && (
          <div style={{
            ...
            <span style={{ color: "var(--text-2)", fontSize: 13 }}>Pro pick</span>
            <button className="btn accent sm">Unlock · subscribe</button>
          </div>
        )}
